...
export default function PlaylistInvitationsScreen() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['playlist-invitations'],
    queryFn: playlistsApi.getMyInvitations,
  });

  const invalidate = () => {
    queryClient.invalidateQueries({ queryKey: ['playlist-invitations'] });
    queryClient.invalidateQueries({ queryKey: ['playlists', 'mine'] });
  };

  const acceptMutation = useMutation({
    mutationFn: (invitationId: string) => playlistsApi.acceptInvitation(invitationId),
    onSuccess: (_, invitationId) => {
      invalidate();
      const accepted = data?.find((inv) => inv.id === invitationId);
      if (accepted) router.push(`/playlists/${accepted.playlistId}`);
    },
  ...
  const rejectMutation = useMutation({
    mutationFn: (invitationId: string) => playlistsApi.rejectInvitation(invitationId),
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <ThemedView style={styles.container}>
          ...
          <AsyncState
            ...
            emptyMessage="No pending playlist invitations.">
            {(data ?? []).map((invitation) => (
              <ThemedView key={invitation.id} style={[styles.row, { borderColor: colors.border }]}>
                <ThemedView style={styles.rowText}>
                  <ThemedText type="defaultSemiBold">{invitation.playlistName}</ThemedText>
                  <ThemedText style={styles.muted}>from {invitation.ownerUsername}</ThemedText>
                </ThemedView>
