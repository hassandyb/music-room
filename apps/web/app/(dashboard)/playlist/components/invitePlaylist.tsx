...
import { useInviteToPlaylistMutation } from "@/services/playlistApi";
import { useGetFriendsQuery } from "@/services/friendshipApi";
import { toast } from "sonner";

export default function InvitePlaylistDialog({ playlistId }: { playlistId: string }) {
    const [open, setOpen] = useState(false);
    const [searchQuery, setSearchQuery] = useState("");

    const { data: friends, isLoading } = useGetFriendsQuery();
    const [invite] = useInviteToPlaylistMutation();
    ...
    const handleInvite = async (userId: string) => {
        try {
            await invite({ id: playlistId, userId }).unwrap();
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="text-foreground gap-2">
            ...
            <DialogContent className="min-w-[600px] max-h-[90vh] bg-card border-border p-0 overflow-hidden flex flex-col">
                <DialogHeader className="p-6 pb-0">
                    <DialogTitle className="text-xl font-semibold text-foreground">Invite to Playlist</DialogTitle>
                    <DialogDescription className="text-muted-foreground">
                        Only friends can be invited to collaborate on a playlist
                    </DialogDescription>
                </DialogHeader>
